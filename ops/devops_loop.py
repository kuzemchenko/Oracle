...
def calibration_state():
    ...
    try:
        ...
        return {"brier_денежный": s.get("brier"), "исходов": s.get("всего_исходов"),
                ...
                "KILL_проверка": s.get("KILL_проверка") or {}}
...
def generate_proposals(metrics, calib):
    ...
    k = calib.get("KILL_проверка") or {}
    if k.get("kill"):   # §11: закрытие/перезапуск проекта — решение ВЛАДЕЛЬЦА (автономия 0)
        props.append({
            "наблюдение": "детерминир. KILL §11 СРАБОТАЛ: " + "; ".join(k.get("reasons") or []),
            "предложение": "решение §11 (закрыть/перезапустить с новой гипотезой edge) — только подпись владельца",
            "эффект": "защита от sunk cost", "требует_подписи": True})
...
def _markdown(report):
    ...
    c = report["калибровка"]
    k = c.get("KILL_проверка") or {}
    kill_txt = ("🛑 СРАБОТАЛ: " + "; ".join(k.get("reasons") or [])) if k.get("kill") else "чисто"
    L.append(f"**Калибровка**: Brier денеж={c.get('brier_денежный')}, "
             ...
             f"до ворот 270: {c.get('до_ворот_270')}. **KILL §11**: {kill_txt}.")
