...
def _strength_from_p(p):
    """Этап2: бирка статистической силы КАНДИДАТА по номинальному p. Ярлык честности, НЕ фильтр
    (ничего не отсекает). Бакеты внутри уже-заметного (p<0.05): «сильный» ≪ «средний» ≪ «слабый».
    None → нет p (не размечаем).

    ВАЖНО (stage-review Этап2, честность бирки): это АБСОЛЮТНАЯ номинальная шкала p, ОДНА для всех
    каналов — не «каждый против своего фона». У трендового p структурный пол 1/(n+1) (≈0.009 при
    ~110 точках фона), поэтому тренд практически не достигает «сильный» (порог 0.005): бирка сравнима
    ПО КАНАЛУ, но между цена/тренд систематически строже к тренду. Для «Разбора дня» это учтено —
    бирка показывается как грубый ориентир силы, а не как межканальный рейтинг."""
    if p is None:
        return None
    if p < 0.005:
        return "сильный"
...
def _tag_news_strength(news_events):
    """Этап2: бирка силы новостного кластера ОТНОСИТЕЛЬНО дня (по салиентности = число статей).
    Верхняя треть диапазона — «сильный», средняя — «средний», нижняя — «слабый». Ярлык, не фильтр.

    stage-review Этап2 (честность): при ЕДИНСТВЕННОМ кластере или равной салиентности всех (диапазон
    вырожден, hi==lo) относительная доля не определена — НЕ клеим принудительный «слабый» (это лгало бы
    про сильное одиночное событие), а помечаем «н.д.» (сила не измерена относительно дня)."""
    sal = [ne.get("салиентность") or 0 for ne in news_events]
    if not sal:
        return
    lo, hi = min(sal), max(sal)
    if hi == lo:                                   # вырожденный день: относительная сила не определена
        for ne in news_events:
            ne["сила_сигнала"] = None
        return
    rng = hi - lo
    for ne in news_events:
        frac = ((ne.get("салиентность") or 0) - lo) / rng
        ne["сила_сигнала"] = "сильный" if frac >= 2 / 3 else "средний" if frac >= 1 / 3 else "слабый"
...
def scan_events(news=None, trends_rows=None, indicators=None, q_max=0.1, tail_df=None,
    ...
    for i, s in enumerate(statistical):
        ...
        # Этап2: бирка статистической силы по каналу (ярлык честности, НЕ фильтр — ничего не отсекает).
        s["сила_сигнала"] = _strength_from_p(s.get("_p_raw"))
    ...
    _tag_news_strength(news_events)                       # Этап2: бирка силы новостей (по салиентности)
    ...
    for s in statistical:
        if s.get("кандидат"):
            ...
            events.append({"вид": s["вид"], "метка": label, "q_value": s["q_value"],
                           "сигнал_после_FDR": s["сигнал_после_FDR"],
                           "сила_сигнала": s.get("сила_сигнала"), "сырое": s})
    for ne in news_events:
        events.append({"вид": "news", "метка": " ".join(ne["ключи"][:3]),
                       "салиентность": ne["салиентность"],
                       "сила_сигнала": ne.get("сила_сигнала"), "сырое": ne})
    ...
    return {
        ...
        **({"протухшие_бары": stale} if stale else {}),
        ...
        "ограничение_П8": (
            ...
            "ПУТИ ДО СУДА (Этап2, честно): ЦЕНОВОЙ кандидат — напрямую (несёт символ) в источники шока "
            "и активацию цепочек; ТРЕНДОВЫЙ — ТОЛЬКО через маппинг ключ→тема→proxy_etf (event_first."
            "_trend_proxy_syms); незамапленный трендовый кандидат помечен, но до суда НЕ доходит (П8). "
            "«кандидатов_к_суду» = помечено кандидатом; сколько трендовых реально замаплено — в протоколе "
            "прогона (трендовый_роутинг). Кластеризуются только латиница/кириллица (en/ru) — CJK/арабские "
            "заголовки как шум для US-универсума выпадают."),
    }
