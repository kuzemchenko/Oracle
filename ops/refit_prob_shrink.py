...
# ЗАВИСИМОСТЬ Д2 (ROADMAP_2026-07, этап Д2 п.4/п.5; комментарий БЕЗ изменения поведения):
# действующие p0/λ knowledge/prob_shrink.yaml (λ=0.0, p0=0.431, подпись П-1 09.07) подогнаны
# по исходам, накопленным ДО аудита Д2 калибровочного трека, — p0/λ подлежат пере-подгонке
# после накопления чистых пост-Д2 исходов. Вердикт Д2 (13.07, ops/reports/d2_diagnosis):
# баг СВЕРКИ НЕ подтверждён (252/252 исхода воспроизведены из сырых котировок бит-в-бит),
# поэтому пауза авто-refit цикла 01.08 по решению владельца 13.07 (Вопрос 3) НЕ активируется —
# цикл идёт как подписано 09.07. (Замечание для полноты: calibration-трек в TRACKS не входит,
# так что даже найденный там баг влиял бы на p0/λ только через money/provisional.)
