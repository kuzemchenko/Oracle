...
def _noise_lesson_node(protocol):
    ...
    ключ = s.get("ключ")
    p = s.get("p_value")
    _n_chance = int(round(1.0 / p)) if isinstance(p, (int, float)) and p > 0 else None
    причина = (f"Всплеск заметен, но при чистой случайности движение такого размера бывает примерно "
               f"1 раз на {_n_chance} проверок — а за день система делает десятки замеров, поэтому "
               "один такой всплеск ожидаем. Это фон, а не событие (не прошёл контроль ложных открытий)."
               if _n_chance else
