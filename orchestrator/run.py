...
def _run_masked(args):
    ...
    if "ОТКАЗ_бюджет" in s:
        ...
        return 1
    if "ОСТАНОВ_бюджет" in s:
        # ревью 04.07 H4: стоп-на-лету (§24) возвращает ОСТАНОВ_бюджет БЕЗ ключа 'агрегат' —
        # раньше падали KeyError-трейсбеком в cron (funnel-путь оба ключа уже обрабатывал)
        print(f"[{s['run_id']}] {s.get('вывод', 'ОСТАНОВ по бюджету на лету (§24)')}")
        return 1
    agg = s["агрегат"]
...
def _run_resolve(args):
    ...
    if k.get("kill"):
        ...
        print("  KILL §11: чисто (порог достигнут; калибровка в норме, edge/бенчмарк §30 не измеряется — FГ)")
    else:
        print(f"  KILL §11: чисто (до порога {pp.get('порог')} разрешённых прогнозов — не применим)")
    if s["ошибок"]:
...
def _run_event_first(args):
    ...
    if p.get("ОТКАЗ_бюджет") or p.get("ОСТАНОВ_бюджет"):   # F0#9 пред-проверка / долг[HIGH] стоп-на-лету
        d = p.get("ОТКАЗ_бюджет") or p["ОСТАНОВ_бюджет"]
        что = "ОТКАЗ (пред-проверка)" if p.get("ОТКАЗ_бюджет") else "ОСТАНОВ на лету"
        print(f"[{p['run_id']}] EVENT-FIRST · {что} по бюджету (§24/Инв#5)")
    ...
    # БАГ#13: headline = РЕАЛЬНЫЙ поток (картограф + граф-топ), а не пустой под --vet легаси-контур.
    fl = p.get("поток_идей") or {}
    if fl:
        контур_txt = (f"{fl['состязательный_контур']} идей" if fl.get("контур_включён")
                      else "выключен (--vet)")
        _overlap = fl.get("пересечение") or 0
        _bd = (f"картограф {fl['картограф']} + граф-топ {fl['граф_топ']}"
               + (f", −{_overlap} пересеч." if _overlap else ""))
        print(f"  поток идей: {fl['всего']} уникальных ({_bd}) "
              f"· треки money {fl['money']}/провиз {fl['провизорный']}/дайдж {fl['дайджест']} "
              f"· состязательный контур: {контур_txt}")
    print(f"  Итог: {p['итог']}")
    return 0
...
def main(argv=None):
    ...
    if args.mode == "edge_forward":
        ...
        print(f"[{r['run_id']}] B4 форвард-тест рёбер: библиотека {и['рёбер_в_библиотеке']} · "
              ...
              f"кулдаун {и['кулдаун_pending']} · шок< {и['шок_под_порогом']} · "
              f"реверс/шум {и['реверс_или_шум_P1#5']} · edge< {и['спит_под_порогом']} · "
              f"нет шока {и['нет_шока_источника']} · дубль {и['дубль_пропущен']}")
        return 0
