...
class MockClient(BaseClient):
    ...
    def _synthesize(self, agent_id, output_kind, user):
        ...
        if output_kind == "world_map":
            # Э4(а) «перебор мира»: детерминированная валидная карта СЕГМЕНТОВ без тикеров и
            # без LLM-величин (кроме структурного порядка) — дымовой тест формы/потока, не качества.
            return {
                "событие": "mock-событие (дымовой тест карты мира)",
                "сегменты": [
                    {"сегмент": "Электрооборудование и трансформаторы", "порядок": 2,
                     "направление": "рост", "канал": "capex-спрос",
                     "механизм": "событие поднимает спрос на энергоинфраструктуру [mock]",
                     "секторы": ["Industrials"], "индустрии": []},
                    {"сегмент": "Электротехническая сталь", "порядок": 3,
                     "направление": "рост", "канал": "цепочка поставок",
                     "механизм": "спрос на трансформаторы тянет GOES-сталь [mock]",
                     "секторы": ["Basic Materials"], "индустрии": []},
                ],
                "обоснование": "mock: реальное рассуждение не выполнялось (дымовой тест формата)",
                "уверенность": "низкая",
            }
