...
def write(result):
    ...
    # Проверка устойчивости к порогу (рамка 3): вердикт КАНОНА — 0.5σ. Пара, у которой вердикт
    # «установлено/не установлено» МЕНЯЕТСЯ поперёк 0.4σ/0.5σ/0.6σ, — НЕУСТОЙЧИВА: это НЕ норма,
    # а КОНТРСВИДЕТЕЛЬСТВО (перенос зависит от порога → walk-forward-неубедителен). Пары, установленные
    # ТОЛЬКО на 0.6σ (не на каноне), — не «сильная связь», а именно такой артефакт (SPY→DBC, XOM→EEM).
    unstable = []
    for row in result["robustness"]:
        statuses = {row[f"{frac}σ"]["status"] for frac in ROBUSTNESS_FRACS}
        if len(statuses) > 1:
            unstable.append(f"{row['источник']}→{row['узел']}")
    lines += [
        "", "## Проверка устойчивости выводов к порогу (0.4σ / 0.5σ / 0.6σ)", "",
        "Вердикт КАНОНА — столбец **0.5σ** (порог подписан владельцем 13.07, зафиксирован ДО прогона; "
        "это проверка устойчивости, НЕ подбор порога — рамка 3).", "",
        "⚠ **НЕУСТОЙЧИВО = КОНТРСВИДЕТЕЛЬСТВО**, а не норма: если вердикт «установлено/не установлено» "
        "меняется поперёк порогов, перенос порого-зависим и walk-forward-неубедителен. Такая пара НЕ "
        "может считаться установленной по каноническому порогу; «установлено только на 0.6σ» — тот же "
        "артефакт (не «сильная связь»).", "",
        (f"Неустойчивых пар: **{len(unstable)}** — " + ", ".join(unstable) if unstable
         else "Неустойчивых пар нет: вердикт каждой пары одинаков на всех трёх порогах."), "",
    ...
    for row in result["robustness"]:
        ...
        pair = f"{row['источник']}→{row['узел']}"
        flag = "⚠ НЕУСТОЙЧИВО (контрсвидетельство)" if pair in unstable else "устойчиво"
        lines.append(f"| {pair} | " + " | ".join(cells) + f" | {flag} |")
    lines += ["", "_Итоговый вердикт пары — по каноническому 0.5σ (колонка результатов выше). "
              "Неустойчивые пары не засчитываются как установленный перенос независимо от того, "
              "на каком пороге они «пролезают» (рамка 3)._", "",
              "_Полные пофолдовые детали — в report.json; YAML-артефакт "
