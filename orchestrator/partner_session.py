...
П2б ПОДКЛЮЧЁН (§R4.4, подпись владельца 05.07): порядок ПОКАЗА идей сессии = продуктовый ранг
«неочевидность × свежесть × доказуемость» (orchestrator/product_rank.py, детерминированный).
Слой Б: пере-ранжируется ТОЛЬКО показ — боевой отбор графа, треки, суд и скоринг §7/веса §30.6
не тронуты; при равном балле сохраняется прежний порядок money → провизорный → картограф.
...
def _idea_from_brief(brief, суд):
    ...
    except TypeError:                      # кросс-ревью №4: нехэшируемый «актив» (list) — не ключ
        v = {}
    if v:
        суд_блок = {"исход": v.get("исход"), "кто_продаёт_нам": v.get("кто_против"),
                    "почему_возможность": v.get("почему_возможность"),
                    "балл": v.get("балл"), "примечание": v.get("примечание")}
        if not v.get("исход"):
            # stage-review LOW-1: запись суда без исхода — честная пометка, а не молчание
            суд_блок["пометка"] = "суд не вынес исхода"
    else:
        суд_блок = {"исход": None, "пометка": "слепой суд по этой идее не гонялся"}
    return {
        "актив": asset,
        "направление": brief.get("направление"),
        "источник": "граф каскадов" + (" (провизорный)" if brief.get("провизорный") else " (money-трек)"),
        "аргумент": {
            # stage-review П3 HIGH-1: «новость → цепочка → компания» — событие-повод и якорь
            # корневого шока ОБЯЗАНЫ доехать до владельца (в _node_brief они есть)
            "событие": brief.get("событие"), "якорь": brief.get("якорь"),
...
def build_session(protocol, *, asof, n_max=SESSION_MAX):
    ...
    Порядок ПОКАЗА = продуктовый ранг П2б (§R4.4, подпись 05.07): «неочевидность × свежесть ×
    доказуемость», при равенстве — прежний порядок money → провизорный → картограф. Дедуп по
    активу (вхождение из старшего трека богаче). Отбор/скоринг §7 не тронуты (слой Б)."""
    if protocol is None:
        return {"ОТКАЗ": "нет ни одного протокола прогона — сначала /run-funnel"}
    if not isinstance(protocol, dict):
        # кросс-ревью №5/№6: валидный JSON с корнем-списком/строкой/пустышкой ([] "" 0) — не
        # протокол; отказ различает «нет протоколов» (None) и «неформатный протокол» честно
        return {"ОТКАЗ": "последний протокол не того формата (корень не объект) — запусти /run-funnel"}
    ...
    def _add(idea):
        nonlocal dups
        a = idea.get("актив")
        if not isinstance(a, str) or not a.strip():   # кросс-ревью №4: не-строковый актив — не идея
            return
        if a in seen:
            dups += 1
    ...
    # П2б (§R4.4, подпись 05.07): ПОКАЗ сортируется продуктовым рангом «неочевидность × свежесть ×
    # доказуемость» (стабильно: при равенстве — прежний порядок money→провизорный→картограф).
    # Это слой Б: боевой отбор/треки/суд/§7 не тронуты — пере-ранжируется только выдача сессии.
    from orchestrator import product_rank as PR
    ideas = PR.annotate_and_sort(ideas)[:n_max]
    return {
        "ts": asof,
        ...
        "идеи": ideas,
        "spec_ref": "§17.6 сессия партнёра (§R3); порядок показа = продуктовый ранг П2б (§R4.4, "
                    "подпись 05.07); отбор/скоринг §7 не тронуты",
    }
