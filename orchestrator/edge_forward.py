...
CANDIDATES = ROOT / "knowledge" / "edge_candidates.jsonl"   # Э4(ж): кандидат-рёбра «перебора мира»
...
def candidate_edges(path=CANDIDATES):
    """Э4(ж), решение владельца 13.07 №7: кандидат-рёбра «перебора мира» из append-only реестра
    knowledge/edge_candidates.jsonl (пишет orchestrator/world_enum.append_edge_candidates).
    Битые строки пропускаются (П8: не чиним), дедуп по (from,to,lag) внутри реестра."""
    p = pathlib.Path(path)
    if not p.exists():
        return []
    seen, edges = set(), []
    for line in p.read_text(encoding="utf-8").splitlines():
        ...
        try:
            ...
            continue
        up, down = r.get("from"), r.get("to")
        if not up or not down or up == down:
            continue
        key = (up, down, int(r.get("lag") or 0))
        if key in seen:
            continue
        seen.add(key)
        edges.append({"from": up, "to": down, "lag": key[2], "origin": "world_enum"})
    return edges


def edge_library(path=SENS, candidates_path=CANDIDATES):
    ...
    Э4(ж): рёбра-кандидаты «перебора мира» (candidates_path, origin=world_enum) участвуют в
    ежедневном форвард-тесте НАРАВНЕ с библиотечными; дедуп против библиотеки (библиотека
    первична). candidates_path=None — только библиотека (для дедупа в самом world_enum)."""
    ...
    for e in entries:
        ...
        if key in seen:
            continue
        seen.add(key)
        edges.append({"from": up, "to": down, "lag": lag, "origin": "library"})
    if candidates_path is not None:
        for c in candidate_edges(candidates_path):
            key = (c["from"], c["to"], c["lag"])
            if key in seen:
                continue
            seen.add(key)
            edges.append(c)
...
def run_edge_forward(*, write=True, seal=True, con=None, now_dt=None,
                     sens_path=SENS, candidates_path=CANDIDATES, db=None, predictions_path=None,
    ...
    edges = edge_library(sens_path, candidates_path=candidates_path)
    own = con is None
    if con is None:
    ...
    try:
        for edge in edges:
            rec = {"ребро": FP.edge_key(edge["from"], edge["to"], edge["lag"]),
                   "origin": edge.get("origin", "library")}   # Э4(ж): world_enum-кандидаты видимы
            ...
            # Э4(ж)/долг B4(а): identity ЭПИЗОДА шока = asof последнего бара окна активации.
            # promote_edges дедупит корм промоушена по (ребро, эпизод) — несколько печатей одного
            # эпизода не считаются несколькими независимыми свидетельствами в биномтест §10.
            spec["episode"] = asof
            spec["edge_origin"] = edge.get("origin", "library")
