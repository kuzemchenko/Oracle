...
def _strength_from_p(p):
    """Этап2: бирка статистической силы КАНДИДАТА по номинальному p (цена/тренды — каждый против
    своего фона). Ярлык честности, НЕ фильтр (ничего не отсекает). Бакеты внутри уже-заметного
    (p<0.05): «сильный» ≪ «средний» ≪ «слабый». None → нет p (не размечаем)."""
    if p is None:
        return None
    if p < 0.005:
        return "сильный"
