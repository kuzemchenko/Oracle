...
def run_event_first(mode="mock", k=3, horizon_days=5, write=True, run_id=None, skip_contour=False,
    ...
    поток_n = len(картограф_идеи) + граф_топ_n
    ...
    protocol = {
        ...
        "итог": (f"идей в поток {поток_n} (картограф {len(картограф_идеи)} + граф-топ {граф_топ_n}); "
    ...
    if write:
        ...
        (LOGS / f"{run_id}.json").write_text(json.dumps(protocol, ensure_ascii=False, indent=2),
                                             encoding="utf-8")
    PROG.finish(f"Идей в поток {поток_n} (картограф {len(картограф_идеи)} + граф-топ {граф_топ_n}) · "
