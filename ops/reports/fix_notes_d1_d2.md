...
## Пересчёт отчётов на честных данных (пункты 12, 16 задания)

Драйверы прогнаны против БОЕВОЙ БД read-only (`file:…?mode=ro`); журналы не тронуты;
config/thresholds.yaml НЕ трогался (драйвер без --write). Артефакты — только в ops/reports/.

### 12 — replay на pre-window df: ВЕРДИКТ ГЕЙТА ИЗМЕНИЛСЯ (честно фиксирую)
Прогон: `ops/calibrate_fdr_background.py --db …/storage/oracle.db` (dry-run) →
`ops/reports/fdr_replay/tail_df_prewindow.json`; затем `ops/replay_scan.py --db …` на этом
артефакте. Результаты (`ops/reports/fdr_replay/REPORT.md`):
- **Все 22 дня окна 21.06–12.07 → 0 сигналов после FDR у ОБЕИХ конфигураций** (старые
  df-константы и новый pre-window t-df). На событийных днях (21–22.06, 11–12.07) сырых
  сигналов много (цена 26→450, тренды 6→17), но FDR не пропускает НИ ОДНОГО.
- Проверка верности реконструкции: ре-прогон 22.06 «почти нормалью» (df=10⁶) даёт **6**
  после FDR против **21** в живом журнале (SYNC §4.2). Разрыв ВЫРОС относительно исходного
  Д1-прогона именно потому, что честные фиксы убрали то, что раздувало живой счёт:
  volume=0-артефакты (#8), BH на округлённом p (#5), asof-тренды с look-ahead (#4),
  протухшие бары (#8). Т.е. часть исходной «21» — артефакты фида/округления, не рынок.
- **Следствие:** исходное утверждение Д1-гейта «новый t-хвост оживляет статистические
  сигналы на событийных днях» на look-ahead-free pre-window replay НЕ подтверждается: новый
  t-хвост (тяжелее нормали → p БОЛЬШЕ → консервативнее) даёт 0, как и старые константы.
  Пороги НЕ трогались (рамка 3): это честный измеренный исход, а не повод тюнить q/df.
  Решение о ре-активации Д1 и трактовке гейта — за основной сессией/владельцем; здесь только
  честно зафиксирован изменившийся вердикт.
- Гард стабильности (#2) на реальных данных поймал 1 расхождение full-vs-pre-window:
  `ABNB.US vol_z_log_20` df 8 (полная история) → 6 (≤2026-06-20) — прежний гард это НЕ ловил
  бы (df менялся, но у обоих-пиннутых — этот случай он видел; здесь важна сама фиксация).

### 16 — пересчёт ops/reports/d2_diagnosis/: выводы НЕ изменились
Прогон `ops/diagnose_calibration.py` против боевых журналов+БД (read-only):
- **баг сверки НЕ подтверждён** (0 расхождений σ_H/порог/бар/исход) — как и было.
- **целостность журнала (#14): 252 resolved, 0 unmatched, ОК** — знаменатель честен, дефекта
  целостности на боевых данных нет (фикс #14 подтверждает 252/252, а не молчит).
- hit как записано = 0.3611; кластерный p = **0.071** против наивного биномиального 6.1e-06
  (завышение значимости ~4 порядка) — вывод «p-кластерный, не систематический баг» ЦЕЛ.
- Структура null (#15) теперь ИЗ ЖУРНАЛА: 7 батчей, сдвиги **[0,3,6,7,10,12,15]** (реальные
  даты якорей), эффективный N=252 — против прежнего хардкода сдвигов [0,3,5,7,9,12,14].
  Реальные сдвиги отличаются от хардкода, но качественный вывод (p_cluster≈0.07 ≫ наивный) тот же.
