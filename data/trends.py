...
def fetch_keyword(keyword, geo="", timeframe="today 3-m"):
...
def store(con, rows_iot, rows_related):
    now = nc.now_utc_iso()
    if rows_iot:
        con.executemany(
            "INSERT OR REPLACE INTO trends (keyword,geo,date,interest,is_partial,source,fetched_at)"
            " VALUES (?,?,?,?,?, 'google_trends', ?)",
            [(*r, now) for r in rows_iot],
        )
    if rows_related:
        con.executemany(
...
def load_keywords():
    ...
    tr = cfg.get("trends", {})
    timeframe = tr.get("timeframe", "today 3-m")
    geos = tr.get("geos", [""])
