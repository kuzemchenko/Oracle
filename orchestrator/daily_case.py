...
def _build_case(status, n, court, rid, name_fn, дата=None):
    ...
    статус_человек = (_LIVE_ЧЕЛОВЕК[_court_state(court)] if status == "live_candidate"
                      else СТАТУС_ЧЕЛОВЕК.get(status, status))
    return {
        "статус": status,
...
def _noise_lesson_node(protocol):
    ...
    ключ = s.get("ключ")
    p = s.get("p_value")
    причина = (f"Всплеск заметен, но при чистой случайности движение такого размера возникало бы "
               f"примерно в {p:.0%} проверок — при десятках замеров за день это ожидаемый фон, а не "
               "событие (не прошёл контроль ложных открытий)."
