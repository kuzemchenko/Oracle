...
def format_partner_session(session, metrics=None):
    ...
    for i, idea in enumerate(session.get("идеи", []), 1):
        ...
        суд = idea.get("суд") or {}
        if суд.get("исход"):
            L.append(f"   ⚖️ Слепой суд: {суд['исход']}" +
                     (f" (балл {суд.get('балл')})" if суд.get("балл") is not None else ""))
            if суд.get("кто_продаёт_нам"):
                L.append(f"   🥊 Кто продаёт нам и почему неправ: {_trunc(суд['кто_продаёт_нам'], 200)}")
            if суд.get("почему_возможность"):
                L.append(f"   ⏳ Почему возможность ещё жива: {_trunc(суд['почему_возможность'], 200)}")
