...
# Числовые поля в сегменте запрещены (рамка 2), КРОМЕ структурного порядка каскада.
_ALLOWED_NUMERIC_KEYS = {"порядок"}
...
def _segment_problems(seg, idx):
    ...
    # рамка 2: числовые величины LLM запрещены (кроме структурного порядка)
    for k, v in seg.items():
        if k in _ALLOWED_NUMERIC_KEYS:
            continue
        if isinstance(v, bool):
            continue
        if isinstance(v, (int, float)):
            problems.append(f"сегмент#{idx}: числовое поле {k!r}={v!r} — LLM-величины запрещены (рамка 2)")
