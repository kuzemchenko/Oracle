...
- **Голосовые сообщения** (12.07): распознаю речь (ops/bot_voice.py — OGG из Telegram →
  [ffmpeg → mp3] → OpenRouter, роль `voice_transcriber` в models.yaml), показываю «🎙 Услышал: …»
  и выполняю как обычный текст («дай идеи», «разбери GEV», мотив к решению, вопрос словами).
  Гейты как у диалога: бюджет §11 (строки в costs.jsonl, agent=voice_transcriber) и ключ OpenRouter.
  Неразборчиво → честное «не разобрал» (П8), команду не выдумываю. РЕКОМЕНДОВАНО на сервере:
  `apt install -y ffmpeg` (без него шлём модели сырой OGG — сработает не у всех моделей цепочки).
